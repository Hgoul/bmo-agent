...
WHISPER_MODEL_SIZE = "base.en"  # tiny.en is faster/less accurate, small.en is slower/more accurate
...
class BotGUI:
    ...
    def __init__(self, master):
        ...
        print("[INIT] Loading Whisper STT model...", flush=True)
        self.whisper_model = None
        try:
            self.whisper_model = WhisperModel(WHISPER_MODEL_SIZE, device="cpu", compute_type="int8")
            print("[INIT] Whisper STT model loaded.", flush=True)
        except Exception as e:
        ...
        def _update():
            ...
            if self.current_state != state:
                if state == BotStates.ERROR:
                    threading.Thread(target=lambda: self.play_sound(self.get_random_sound(error_sounds_dir)), daemon=True).start()
    ...
    def transcribe_audio(self, filename):
        ...
        if not self.whisper_model:
            print("[ERROR] Whisper model not loaded.", flush=True)
            return ""
        try:
            segments, info = self.whisper_model.transcribe(filename, language="en", beam_size=1)
    ...
    def capture_image(self):
        ...
        time.sleep(0.4)
        try:
            subprocess.run(["rpicam-still", "-t", "500", "-n", "--width", "640", "--height", "480", "-o", BMO_IMAGE_FILE], check=True, timeout=5)
